	...
	GETACL3resok *objp;
{
	...
	if (!xdr_array(xdrs, (char **)&objp->ace.ace_val, (u_int *)&objp->ace.ace_len, ~0, sizeof(nfsacl_ace), xdr_nfsacl_ace)) {
		return (FALSE);
	}
	if (!xdr_u_int(xdrs, &objp->default_ace_count)) {
		return (FALSE);
	}
	if (!xdr_array(xdrs, (char **)&objp->default_ace.default_ace_val, (u_int *)&objp->default_ace.default_ace_len, ~0, sizeof(nfsacl_ace), xdr_nfsacl_ace)) {
		return (FALSE);
	}
	return (TRUE);
}
