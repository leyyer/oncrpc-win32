...
nfs_program_3(struct svc_req *rqstp, SVCXPRT *transp)
{
	...
	memset(&argres, 0, sizeof argres);
	result = (*local)(&argument, &argres, &local_free);
	if (result == 0) {
		if (!svc_sendreply(transp, xdr_result, &argres)) {
...
nfsacl_program_3(struct svc_req *rqstp, SVCXPRT *transp)
{
	...
	union {
		GETACL3res  nfsacl3_getacl_3_res;
		SETACL3res  nfsacl3_setacl_3_res;
	} argres;
	int result;
	bool_t (*xdr_argument)(), (*xdr_result)();
	int (*local)();
